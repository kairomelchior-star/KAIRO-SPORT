...
async function loadAutomaticNews(){
  const grid=document.getElementById('auto-news-grid'),status=document.getElementById('auto-news-status'); if(!grid)return;
  try{
    const response=await fetch('data/news.json?v='+Date.now(),{cache:'no-store'}); if(!response.ok)throw new Error('news.json indisponible');
    const data=await response.json(),items=Array.isArray(data.items)?data.items:[];
    if(!items.length){grid.innerHTML='<p class="auto-news-loading">Les prochaines actualités apparaîtront automatiquement ici.</p>';return;}
    grid.innerHTML=items.slice(0,6).map(item=>{
      ...
      const safeTitle=(item.title_fr||item.title||'Actualité football').replace(/</g,'&lt;').replace(/>/g,'&gt;');
      const safeDesc=(item.description_fr||item.description||'').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    ...
    if(data.updated_at){const updated=new Date(data.updated_at);status.textContent='Actualisé le '+updated.toLocaleString('fr-FR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'});}
  }catch(error){grid.innerHTML='<p class="auto-news-loading">La mise à jour automatique est momentanément indisponible.</p>';status.textContent='En attente de mise à jour';console.error(error);}
}
loadAutomaticNews();
